-- Allow main + restricted admin PIN to load/send/review chat (same pattern as dars bootstrap).

create or replace function private.mdr_resolve_chat_admin(p_actor_id uuid, p_pin text)
...
begin
  ...
  if v_admin then
    ...
    v_from_name := coalesce(nullif(btrim(v_actor.name), ''), v_from_name);
    ...
    values (p_thread_id, v_actor.id, 'admin', v_from_name, v_body, true, false, p_request)
...
declare
  ...
  v_reviewer text := 'জিম্মাদার';
begin
  v_actor := private.mdr_resolve_chat_admin(p_actor_id, p_pin);
  if v_actor.id is null then
    return jsonb_build_object('ok', false, 'error', 'invalid_pin');
  end if;
  v_reviewer := coalesce(nullif(btrim(v_actor.name), ''), v_reviewer);
