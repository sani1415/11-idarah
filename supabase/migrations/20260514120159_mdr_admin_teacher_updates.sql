-- Admin dashboard bootstrap for teacher-updated records.
-- Keeps the admin-visible class overlay/recent feed database-backed instead of
-- depending on stale browser-local caches.

create or replace function private.mdr_admin_dashboard_actor(
  p_actor_id uuid,
  p_pin text
)
...
revoke execute on function private.mdr_admin_dashboard_actor(uuid, text) from public, anon, authenticated;
