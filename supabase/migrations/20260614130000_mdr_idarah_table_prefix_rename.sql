...
-- Pattern: dept_x -> mdr_dept_x, khedmat_x -> mdr_khedmat_x, shared_x -> mdr_shared_x.
-- Existing mdr_* tables stay unchanged. Waqf waqf_* tables stay unchanged.
...
-- 2) Patch function bodies (public + private): qualified and bare type/table refs
do $$
declare
  r record;
  def text;
  ...
  tables text[] := array[
    ...
    'dept_transaction_items',
    'dept_edit_requests',
    'dept_extra_fields',
    'dept_departments',
    'khedmat_activity_types',
    'khedmat_beneficiaries',
    'khedmat_daily_logs',
    'shared_push_subscriptions',
...
-- 3) Rename dept/khedmat RPC entrypoints for Idarah frontend consistency
